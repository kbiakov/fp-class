...
instance Show Expr where
  show (I x) = show x
  show (x `Add` y) = show x ++ "+" ++ show y
  show ((x `Add` y) `Mul` (v `Add` w)) = "(" ++ show (x `Add` y) ++ ")*(" ++ show (v `Add` w) ++ ")"
  show ((x `Add` y) `Mul` z) = "(" ++ show (x `Add` y) ++ ")*" ++ show z
  show (x `Mul` (y `Add` z)) = show x ++ "*(" ++ show (y `Add` z) ++ ")"
  show (x `Mul` y) = show x ++ "*" ++ show y
